...
top_level_unique_values_dict = {
    "45c4f65f-d2cc-460a-a3cf-bd32794756bf": pickle.loads(
        b"\x80\x04\x95\x0f\x00\x00\x00\x00\x00\x00\x00\x8c\x0ba small cat\x94."
    ),
    "b0fe2cab-a90c-4f2c-91c7-e18b61ce798d": pickle.loads(
        b"\x80\x04\x95S\x01\x00\x00\x00\x00\x00\x00XL\x01\x00\x00A small cat is often referred to as a kitten if it is young, or simply a small-sized domestic cat if it is fully grown but petite. Small cats are known for their agility, playful behavior, and affectionate nature. If you want, I can provide information on caring for small cats, different small cat breeds, or anything else related!\x94."
    ),
    "d716d214-35cd-4c36-8155-d5e23f7bb8f4": pickle.loads(
        b"\x80\x04\x95o\x01\x00\x00\x00\x00\x00\x00\x8c%griptape.artifacts.image_url_artifact\x94\x8c\x10ImageUrlArtifact\x94\x93\x94)\x81\x94}\x94(\x8c\x04type\x94\x8c\x10ImageUrlArtifact\x94\x8c\x0bmodule_name\x94\x8c%griptape.artifacts.image_url_artifact\x94\x8c\x02id\x94\x8c 19482b5e2458454c8a45bee46d9ae1db\x94\x8c\treference\x94N\x8c\x04meta\x94}\x94\x8c\x04name\x94h\n\x8c\x16encoding_error_handler\x94\x8c\x06strict\x94\x8c\x08encoding\x94\x8c\x05utf-8\x94\x8c\x05value\x94\x8cRhttp://localhost:8124/static/27065efa-c8e0-4129-a7aa-419b882dc434.png?t=1755102381\x94ub."
    ),
    "f29e8644-90a7-4a03-a18c-980f87bf2691": pickle.loads(
        b"\x80\x04\x95\x10\x00\x00\x00\x00\x00\x00\x00\x8c\x0cgpt-4.1-mini\x94."
    ),
    "79f75ca3-6e5f-46c7-94fa-fc02928119a5": pickle.loads(b"\x80\x04\x95\x04\x00\x00\x00\x00\x00\x00\x00\x8c\x00\x94."),
    "5c7cee06-4e26-4a04-8c81-b2aa051eda42": pickle.loads(b"\x80\x04]\x94."),
    "dfb23919-18cd-4bdd-a06d-0bff8c67d898": pickle.loads(b"\x80\x04]\x94."),
    "0538a850-9895-46de-b7ba-f48faf755ac3": pickle.loads(b"\x80\x04\x89."),
    "44c68b3b-cfed-4211-bd98-d041dbd02440": pickle.loads(
        b"\x80\x04\x95\x0c\x00\x00\x00\x00\x00\x00\x00\x8c\x08dall-e-3\x94."
    ),
    "dca13c47-bca4-42cb-a5d9-17ff684205b8": pickle.loads(
...
with GriptapeNodes.ContextManager().flow(flow0_name):
    node0_name = GriptapeNodes.handle_request(
        CreateNodeRequest(
            ...
            metadata={
                "position": {"x": 321.0677083333334, "y": 685.3333333333333},
                "tempId": "placing-1755102279886-wwehbm",
    ...
    node1_name = GriptapeNodes.handle_request(
        CreateNodeRequest(
            ...
            node_name="End Flow",
            metadata={
                "position": {"x": 1629.0677083333333, "y": 685.3333333333333},
                "tempId": "placing-1755102285321-ds1gk9",
                "library_node_metadata": NodeMetadata(
                    category="workflows",
    ...
    node2_name = GriptapeNodes.handle_request(
        CreateNodeRequest(
            ...
            node_name="Agent",
            metadata={
                "position": {"x": 1019.7343749999998, "y": 194.66666666666674},
                "tempId": "placing-1755102288879-fc9hag",
                "library_node_metadata": NodeMetadata(
                    category="agents",
    ...
    node3_name = GriptapeNodes.handle_request(
        CreateNodeRequest(
            ...
            metadata={
                "position": {"x": 1019.7343749999998, "y": 812.0000000000001},
                "tempId": "placing-1755102324455-3v9t4i",
                "library_node_metadata": NodeMetadata(
                    category="image",
    ...
    GriptapeNodes.handle_request(
        CreateConnectionRequest(
            ...
            source_parameter_name="output",
            target_node_name=node1_name,
            target_parameter_name="output_1",
            initial_setup=True,
        )
    )
    GriptapeNodes.handle_request(
    ...
    with GriptapeNodes.ContextManager().node(node0_name):
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                ...
                value=top_level_unique_values_dict["45c4f65f-d2cc-460a-a3cf-bd32794756bf"],
                initial_setup=True,
                is_output=False,
            )
        )
    with GriptapeNodes.ContextManager().node(node1_name):
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                ...
                value=top_level_unique_values_dict["b0fe2cab-a90c-4f2c-91c7-e18b61ce798d"],
                initial_setup=True,
                is_output=False,
            )
        )
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                parameter_name="output_1",
                node_name=node1_name,
                value=top_level_unique_values_dict["d716d214-35cd-4c36-8155-d5e23f7bb8f4"],
                initial_setup=True,
                is_output=False,
            )
        )
    with GriptapeNodes.ContextManager().node(node2_name):
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                ...
                value=top_level_unique_values_dict["f29e8644-90a7-4a03-a18c-980f87bf2691"],
                initial_setup=True,
                is_output=False,
            )
        )
        ...
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                ...
                node_name=node2_name,
                value=top_level_unique_values_dict["79f75ca3-6e5f-46c7-94fa-fc02928119a5"],
                initial_setup=True,
                is_output=False,
            )
        )
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                ...
                node_name=node2_name,
                value=top_level_unique_values_dict["5c7cee06-4e26-4a04-8c81-b2aa051eda42"],
                initial_setup=True,
                is_output=False,
            )
        )
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                ...
                node_name=node2_name,
                value=top_level_unique_values_dict["dfb23919-18cd-4bdd-a06d-0bff8c67d898"],
                initial_setup=True,
                is_output=False,
            )
        )
        ...
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                ...
                node_name=node2_name,
                value=top_level_unique_values_dict["0538a850-9895-46de-b7ba-f48faf755ac3"],
                initial_setup=True,
                is_output=False,
            )
        )
    with GriptapeNodes.ContextManager().node(node3_name):
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                ...
                value=top_level_unique_values_dict["44c68b3b-cfed-4211-bd98-d041dbd02440"],
                initial_setup=True,
                is_output=False,
            )
        )
        ...
        GriptapeNodes.handle_request(
            SetParameterValueRequest(
                ...
                node_name=node3_name,
                value=top_level_unique_values_dict["dca13c47-bca4-42cb-a5d9-17ff684205b8"],
                initial_setup=True,
                is_output=False,
            )
        )
