...
from griptape_nodes.node_library.library_registry import Dependencies, LibrarySchema
...
class TestDependenciesSchema:
    ...
    def test_library_dependencies_accepts_url_list(self) -> None:
        deps = Dependencies(library_dependencies=["griptape-ai/nodes-opencolorio@v1.2.0"])
        assert deps.library_dependencies == ["griptape-ai/nodes-opencolorio@v1.2.0"]
...
class TestLibraryDependencyProblem:
    ...
    def test_multiple_problems_message(self) -> None:
        problems = [
...
def _make_schema_mock(library_dependencies: list[str] | None) -> MagicMock:
    schema = MagicMock()
    schema.name = "test_lib"
    schema.metadata.library_version = "1.0.0"
    schema.metadata.dependencies.library_dependencies = library_dependencies
    return schema
