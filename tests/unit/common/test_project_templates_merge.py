...
name: "Test Project"
"""
        result = load_partial_project_template(yaml_text)

        assert result.validation.status == ProjectValidationStatus.GOOD
        assert result.overlay.name == "Test Project"
        assert result.overlay.project_template_schema_version == "0.1.0"
        assert result.overlay.situations == {}
        assert result.overlay.directories == {}
        assert result.overlay.environment == {}
        assert result.overlay.description is None
...
"""
        ...
        assert len(merged.situations) == len(DEFAULT_PROJECT_TEMPLATE.situations)
        # Should inherit all directories from base
        assert len(merged.directories) == len(DEFAULT_PROJECT_TEMPLATE.directories)
...
"""
        ...
        assert len(merged.situations) == len(DEFAULT_PROJECT_TEMPLATE.situations) + 1
...
"""
        ...
        base_policy = DEFAULT_PROJECT_TEMPLATE.situations["save_node_output"].policy
...
"""
        ...
        assert len(merged.directories) == len(DEFAULT_PROJECT_TEMPLATE.directories) + 1
