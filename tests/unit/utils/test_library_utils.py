...
class TestIsMonorepo:
    ...
    def test_is_monorepo_returns_false_when_only_one_library_json_found(self, temp_dir: Path) -> None:
        ...
        with patch("griptape_nodes.utils.library_utils.get_git_repository_root") as mock_get_root:
            with patch("griptape_nodes.utils.library_utils.find_all_files_in_directory") as mock_find:
...
class TestCloneAndGetLibraryVersion:
    ...
    def test_clone_and_get_library_version_converts_ssh_to_https(self) -> None:
        ...
        with patch("griptape_nodes.utils.git_utils._is_ssh_url") as mock_is_ssh:
            with patch("griptape_nodes.utils.git_utils._convert_ssh_to_https") as mock_convert:
                with patch("griptape_nodes.utils.library_utils.pygit2.clone_repository") as mock_clone:
                    with patch("griptape_nodes.utils.library_utils.find_file_in_directory") as mock_find:
                        mock_is_ssh.return_value = True
                        mock_convert.return_value = "https://github.com/user/repo.git"
                        mock_repo = Mock()
                        mock_clone.return_value = mock_repo

                        library_json = Path("/tmp/temp/griptape_nodes_library.json")
        ...
        with patch("griptape_nodes.utils.git_utils._is_ssh_url") as mock_is_ssh:
            with patch("griptape_nodes.utils.library_utils.pygit2.clone_repository") as mock_clone:
                with patch("griptape_nodes.utils.library_utils.find_file_in_directory") as mock_find:
                    ...
                    library_json = Path("/tmp/temp/subdir/griptape_nodes_library.json")
                    mock_find.return_value = library_json
