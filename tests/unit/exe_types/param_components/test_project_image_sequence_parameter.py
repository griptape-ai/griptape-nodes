...
from griptape_nodes.exe_types.param_components.project_image_sequence_parameter import (
    _FALLBACK_SEQUENCE_MACRO,
    _build_sequence_destination_from_situation,
)
from griptape_nodes.files.image_sequence import ImageSequenceDestination
...
HANDLE_REQUEST_PATH = (
    "griptape_nodes.exe_types.param_components.project_image_sequence_parameter.GriptapeNodes.handle_request"
)
BUILD_VERSIONED_PATH = (
    "griptape_nodes.exe_types.param_components.project_image_sequence_parameter.build_versioned_sequence_destination"
)
...
class TestBuildSequenceDestinationFromSituation:
    ...
    def test_plain_filename_parsed_into_stem_and_extension(self) -> None:
        situation = _make_situation("{outputs}/{file_name_base}_{frame:04}.{file_extension}")
        success = GetSituationResultSuccess(situation=situation, result_details="ok")
        mock_dest = MagicMock(spec=ImageSequenceDestination)
    ...
    def test_hash_pattern_filename_converted_before_parsing(self) -> None:
        ...
        with (
            ...
            _build_sequence_destination_from_situation("frame_####.exr", "save_image_sequence_frame")

        macro_path = mock_build.call_args.args[0]
        assert macro_path.variables["file_extension"] == "exr"

    def test_extra_vars_forwarded_to_macro(self) -> None:
        situation = _make_situation("{outputs}/{node_name}/{file_name_base}_{frame:04}.{file_extension}")
        success = GetSituationResultSuccess(situation=situation, result_details="ok")
        mock_dest = MagicMock(spec=ImageSequenceDestination)
        ...
        with (
            ...
            _build_sequence_destination_from_situation("frame.exr", "save_image_sequence_frame", node_name="MyNode")

        macro_path = mock_build.call_args.args[0]
        assert macro_path.variables["node_name"] == "MyNode"

    def test_situation_overwrite_policy_forwarded(self) -> None:
        situation = _make_situation("{outputs}/{frame:04}.exr", on_collision="OVERWRITE")
        success = GetSituationResultSuccess(situation=situation, result_details="ok")
        mock_dest = MagicMock(spec=ImageSequenceDestination)
    ...
    def test_situation_create_dirs_forwarded(self) -> None:
        situation = _make_situation("{outputs}/{frame:04}.exr", create_dirs=False)
        success = GetSituationResultSuccess(situation=situation, result_details="ok")
        mock_dest = MagicMock(spec=ImageSequenceDestination)
    ...
    def test_returns_image_sequence_destination(self) -> None:
        situation = _make_situation("{outputs}/{frame:04}.exr")
        success = GetSituationResultSuccess(situation=situation, result_details="ok")
        mock_dest = MagicMock(spec=ImageSequenceDestination)

        with patch(HANDLE_REQUEST_PATH, return_value=success), patch(BUILD_VERSIONED_PATH, return_value=mock_dest):
            result = _build_sequence_destination_from_situation("frame.exr", "save_image_sequence_frame")
