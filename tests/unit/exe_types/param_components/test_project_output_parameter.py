...
HANDLE_REQUEST_PATH = "griptape_nodes.exe_types.param_components.project_output_parameter.GriptapeNodes.handle_request"
OBJECT_MANAGER_PATH = "griptape_nodes.exe_types.param_components.project_output_parameter.GriptapeNodes.ObjectManager"


class _ConcreteParam(ProjectOutputParameter):
...
def _make_connections_result(*connections: IncomingConnection) -> ListConnectionsForNodeResultSuccess:
...
class TestGetUpstreamDestination:
    ...
    def test_returns_none_when_list_connections_fails(self) -> None:
        param = _make_param()
        failure = ListConnectionsForNodeResultFailure(result_details="error")
    ...
    def test_returns_none_when_connection_targets_different_parameter(self) -> None:
        ...
        mock_source = MagicMock(spec=[])  # no attributes

        with (
            patch(HANDLE_REQUEST_PATH, return_value=success),
            patch(OBJECT_MANAGER_PATH) as mock_om,
        ):
    ...
    def test_returns_destination_when_source_has_attribute(self) -> None:
        ...
        conn = _make_connection(target_param="output")
        success = _make_connections_result(conn)
        expected_dest = MagicMock()
        mock_source = MagicMock(spec=["test_destination"])
    ...
    def test_raises_when_provider_attribute_returns_none(self) -> None:
        ...
        with patch(HANDLE_REQUEST_PATH, return_value=success), patch(OBJECT_MANAGER_PATH) as mock_om:
            mock_om.return_value.attempt_get_object_by_name.return_value = mock_source
    ...
    def test_skips_non_provider_and_returns_provider_destination(self) -> None:
        ...
        expected_dest = MagicMock()
        plain_source = MagicMock(spec=[])  # no test_destination
        provider_source = MagicMock(spec=["test_destination"])
    ...
    def test_allowed_modes_default(self) -> None:
        param = _make_param()
        assert param._allowed_modes == {ParameterMode.INPUT, ParameterMode.PROPERTY}

    def test_custom_allowed_modes(self) -> None:
        mock_node = MagicMock()
        mock_node.name = "N"
        param = _ConcreteParam(mock_node, "out", default_value="x", situation="s", allowed_modes={ParameterMode.OUTPUT})
        assert param._allowed_modes == {ParameterMode.OUTPUT}
