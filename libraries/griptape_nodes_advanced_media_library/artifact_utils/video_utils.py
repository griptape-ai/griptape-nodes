...
def frames_to_video_artifact(frames: List[Any], fps: int = 30, video_format: str = "mp4") -> VideoUrlArtifact:
    ...
    try:
        ...
        if len(first_frame_np.shape) == 3 and first_frame_np.shape[2] == 3:
            height, width, _ = first_frame_np.shape
        else:
            raise ValueError("Frames must be RGB images")
        ...
        for frame in frames:
            ...
            if len(frame_np.shape) == 3 and frame_np.shape[2] == 3:
                ...
                raise ValueError("All frames must be RGB images")
...
def numpy_video_to_video_artifact(video_np: Any, fps: int = 30, video_format: str = "mp4") -> VideoUrlArtifact:
    ...
    if not isinstance(video_np, np.ndarray):
        raise ValueError("video_np must be a numpy array")
    
    if len(video_np.shape) != 4:
        raise ValueError("video_np must have shape (frames, height, width, channels)")
