...
class CannyConvertImage(ControlNode):
    def __init__(self, **kwargs) -> None:
        ...
        self.add_parameter(
            Parameter(
                ...
                type="bool",
                allowed_modes={ParameterMode.PROPERTY, ParameterMode.INPUT},
                tooltip="Auto run the node on changes, useful for tuning output.",
            )
        )
        self.add_parameter(
            Parameter(
    ...
    def after_value_set(self, parameter: Parameter, value: Any, modified_parameters_set: set[str]) -> None:  # noqa: ARG002
        ...
        auto_run = bool(self.get_parameter_value("auto_run")) or (parameter.name == "auto_run" and bool(value))
        if auto_run:
