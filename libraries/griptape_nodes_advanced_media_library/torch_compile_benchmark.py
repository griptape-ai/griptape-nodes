...
def benchmark_compilation_strategies():
    ...
    strategies = {
        ...
        "whole_transformer": lambda p: setattr(p, 'transformer', optimizer.compile_transformer(p.transformer, "max-autotune")) or p,
        "individual_layers": lambda p: setattr(p, 'transformer', optimizer.compile_individual_layers(p.transformer, "max-autotune")) or p,
        "attention_only": lambda p: setattr(p, 'transformer', optimizer.compile_attention_layers(p.transformer, "max-autotune")) or p,
    }
