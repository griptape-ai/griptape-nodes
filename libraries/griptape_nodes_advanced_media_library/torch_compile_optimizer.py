...
class TorchCompileOptimizer:
    ...
    def get_compile_config(self, mode: str = "fast") -> dict:
        ...
        """
        configs = {
            "default": {"mode": "default", "dynamic": False, "fullgraph": False, "options": None},
            "reduce-overhead": {"mode": "reduce-overhead", "dynamic": False, "fullgraph": True, "options": None},
            "max-autotune": {
                "mode": "max-autotune",
                "dynamic": False,
    ...
    def compile_transformer(self, transformer_model: torch.nn.Module, mode: str = "fast") -> torch.nn.Module:
        ...
        compile_kwargs = {"dynamic": config["dynamic"], "fullgraph": config["fullgraph"]}
    ...
    def compile_individual_layers(
        self, transformer_model: torch.nn.Module, mode: str = "max-autotune"
    ) -> torch.nn.Module:
...
def create_sample_inputs_for_flux(
    batch_size: int = 1, height: int = 512, width: int = 512, device: str = "cuda"
) -> dict:
    ...
    sample_inputs = {
        "hidden_states": torch.randn(
            batch_size, seq_len, 64, dtype=torch.bfloat16, device=device
        ),  # Reduced channel dim
    ...
    class SimpleTransformer(torch.nn.Module):
        def __init__(self):
            ...
            self.layers = torch.nn.ModuleList(
                [torch.nn.TransformerEncoderLayer(512, 8, batch_first=True) for _ in range(6)]
            )
