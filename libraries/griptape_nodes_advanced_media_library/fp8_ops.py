...
class FP8Linear(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if len(input_shape) == 2:
            ...
            output = torch._scaled_mm(input_fp8, self._cached_weight_t, bias=bias_compute, 
                                     scale_a=self.scale_input, scale_b=self.scale_weight, 
        ...
        if input_shape[-1] != self.in_features:
            raise RuntimeError(f"Input last dimension {input_shape[-1]} doesn't match layer in_features {self.in_features}")
...
def replace_linear_with_fp8(module: nn.Module, name: str = "") -> nn.Module:
    ...
    for attr_name, child in module.named_children():
        if isinstance(child, nn.Linear):
            ...
            fp8_linear = FP8Linear(child.in_features, child.out_features, child.bias is not None, device=child.weight.device)
...
def replace_linear_with_fp8_selective(module: nn.Module, name: str = "") -> nn.Module:
    for attr_name, child in module.named_children():
        if isinstance(child, nn.Linear):
            ...
            if any(skip_name in f"{name}.{attr_name}".lower() for skip_name in 
                   ['embedding', 'embed', 'position', 'token', 'vocab', 'classifier', 'prediction']):
...
class FP8LinearCompatible(FP8Linear):
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        return torch._scaled_mm(input_fp8, self.weight.t(), bias=bias_compute, 
                               scale_a=self.scale_input, scale_b=self.scale_weight, 
...
def replace_attention_layers_with_fp8(module: nn.Module, name: str = "") -> nn.Module:
    ...
    """
    attention_keywords = {'to_q', 'to_k', 'to_v', 'add_q_proj', 'add_k_proj', 'add_v_proj', 'to_out', 'to_add_out', 'attn'}
    ...
    for parent, attr_name, child_module, full_path in modules_to_replace:
        if not isinstance(getattr(parent, attr_name), FP8Linear):  # Don't replace twice
            fp8_linear = FP8Linear(child_module.in_features, child_module.out_features, 
                                 child_module.bias is not None, device=child_module.weight.device)
...
class FP8ElementwiseOps:
    ...
    @staticmethod
    def modulation(x: torch.Tensor, scale: torch.Tensor, shift: torch.Tensor, out_dtype: torch.dtype = None) -> torch.Tensor:
...
class FP8Normalization:
    ...
    @staticmethod
    def layer_norm(x: torch.Tensor, normalized_shape, weight=None, bias=None, eps: float = 1e-5, out_dtype: torch.dtype = None) -> torch.Tensor:
    ...
    @staticmethod
    def rms_norm(x: torch.Tensor, scale: torch.Tensor, eps: float = 1e-6, out_dtype: torch.dtype = None) -> torch.Tensor:
