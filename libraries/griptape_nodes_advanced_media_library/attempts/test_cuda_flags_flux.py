...
def main():
    ...
    # Test configurations
    test_configs = [
        {"name": "Baseline (Default Settings)", "flags": {}},
        {"name": "TF32 Enabled", "flags": {"matmul.allow_tf32": True}},
        {"name": "Flash Attention", "flags": {"enable_flash_sdp": True}},
        {"name": "Memory Efficient Attention", "flags": {"enable_mem_efficient_sdp": True}},
        {"name": "cuDNN Attention", "flags": {"enable_cudnn_sdp": True}},
        {
            "name": "All Attention Optimizations",
            "flags": {"enable_flash_sdp": True, "enable_mem_efficient_sdp": True, "enable_cudnn_sdp": True},
        },
        {
    ...
    for config in test_configs:
        try:
            ...
            results.append({"name": config["name"], "times": times, "avg_time": sum(times) / len(times)})

        except Exception as e:
            print(f"Error testing {config['name']}: {str(e)}")
            results.append({"name": config["name"], "times": [], "avg_time": float("inf"), "error": str(e)})
