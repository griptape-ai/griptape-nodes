...
def benchmark_config(pipe, config_name: str, **kwargs) -> float:
    ...
    print("Warming up...")
    with torch.no_grad():
        _ = pipe(prompt="warmup", num_inference_steps=1, guidance_scale=1.0, height=512, width=512, **kwargs)
...
def test_inference_parameters():
    ...
    for steps in step_counts:
        results[f"steps_{steps}"] = benchmark_config(pipe, f"{steps} Steps", num_inference_steps=steps)

    # Test different guidance scales
    guidance_scales = [0.0, 1.0, 3.5, 7.0]
    for scale in guidance_scales:
        results[f"guidance_{scale}"] = benchmark_config(pipe, f"Guidance {scale}", guidance_scale=scale)
