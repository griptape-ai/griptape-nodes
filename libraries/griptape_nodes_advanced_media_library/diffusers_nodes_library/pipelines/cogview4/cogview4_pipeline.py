...
from diffusers_nodes_library.pipelines.cogview4.cogview4_pipeline_memory_footprint import (
    optimize_cogview4_pipeline_memory_footprint,
)  # type: ignore[reportMissingImports]
from diffusers_nodes_library.pipelines.cogview4.cogview4_pipeline_parameters import (
    Cogview4PipelineParameters,  # type: ignore[reportMissingImports]
)
...
class Cogview4Pipeline(ControlNode):
    """Griptape wrapper around diffusers.pipelines.cogview4.CogView4Pipeline."""

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = Cogview4PipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        self.log_params.append_to_logs("Preparing CogView4 models...\n")
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_cogview4_pipeline_memory_footprint(pipe)

        num_inference_steps = self.pipe_params.get_num_inference_steps()

        def callback_on_step_end(
            pipe: diffusers.CogView4Pipeline,
            i: int,
            _t: Any,
