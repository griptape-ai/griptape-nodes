...
from diffusers_nodes_library.pipelines.ddim.ddim_pipeline_memory_footprint import (
    optimize_ddim_pipeline_memory_footprint,
)  # type: ignore[reportMissingImports]
from diffusers_nodes_library.pipelines.ddim.ddim_pipeline_parameters import (
    DdimPipelineParameters,  # type: ignore[reportMissingImports]
)
...
class DdimPipeline(ControlNode):
    ...
    def _process(self) -> AsyncResult | None:
        ...
        self.log_params.append_to_logs("Preparing DDIM models...\n")
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_ddim_pipeline_memory_footprint(pipe)

        num_inference_steps = self.pipe_params.get_num_inference_steps()

        def callback_on_step_end(
            pipe: diffusers.DDIMPipeline,
            i: int,
            _t: Any,
