...
from diffusers_nodes_library.pipelines.unclip.optimize_unclip_pipeline_memory_footprint import (
    optimize_unclip_pipeline_memory_footprint,  # type: ignore[reportMissingImports]
    print_unclip_pipeline_memory_footprint,  # type: ignore[reportMissingImports]
)
from diffusers_nodes_library.pipelines.unclip.unclip_pipeline_parameters import (
    UnclipPipelineParameters,  # type: ignore[reportMissingImports]
)
...
class UnclipPipeline(ControlNode):
    """Griptape wrapper around diffusers.UnCLIPPipeline."""

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = UnclipPipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        with self.log_params.append_profile_to_logs("Loading model metadata"):
            ...
            pipe: diffusers.UnCLIPPipeline = model_cache.from_pretrained(
        ...
        with (
            ...
            optimize_unclip_pipeline_memory_footprint(pipe)
        ...
        logger.info("UnCLIP memory footprint after inference:")
        print_unclip_pipeline_memory_footprint(pipe)
