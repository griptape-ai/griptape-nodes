...
from diffusers_nodes_library.common.utils.torch_utils import get_best_device, print_pipeline_memory_footprint  # type: ignore[reportMissingImports]
...
class SD35ModelManager:
    ...
    def add_model_parameter(self) -> None:
        ...
        self._repo_param = HuggingFaceRepoParameter(
            node=self._node,
            repo_ids=list(self.MODEL_REPOS.values()),
    ...
    def get_pipeline(self) -> diffusers.StableDiffusion3Pipeline:
        ...
        if cache_key in self._pipeline_cache:
            logger.info(f"Using cached SD3.5 pipeline: {cache_key}")
            return self._pipeline_cache[cache_key]
        
        # Load new pipeline
        logger.info(f"Loading new SD3.5 pipeline: {repo_id}")
        ...
        logger.info(f"Cached SD3.5 pipeline: {cache_key}")
