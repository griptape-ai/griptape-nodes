...
from diffusers_nodes_library.pipelines.ddpm.ddpm_pipeline_memory_footprint import (
    optimize_ddpm_pipeline_memory_footprint,
)  # type: ignore[reportMissingImports]
from diffusers_nodes_library.pipelines.ddpm.ddpm_pipeline_parameters import (
    DdpmPipelineParameters,  # type: ignore[reportMissingImports]
)
...
class DdpmPipeline(ControlNode):
    """Griptape wrapper around diffusers.DDPMPipeline."""

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = DdpmPipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        self.log_params.append_to_logs("Preparing DDPM models...\n")
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_ddpm_pipeline_memory_footprint(pipe)

        num_inference_steps = self.pipe_params.get_num_inference_steps()

        def callback_on_step_end(
            pipe: diffusers.DDPMPipeline,
            i: int,
            _t: Any,
