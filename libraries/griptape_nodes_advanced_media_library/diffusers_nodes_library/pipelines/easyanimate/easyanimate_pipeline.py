...
class EasyanimatePipeline(ControlNode):
    """Griptape wrapper around diffusers.pipelines.easyanimate.EasyAnimatePipeline."""

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = EasyanimatePipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_easyanimate_pipeline_memory_footprint(pipe)
