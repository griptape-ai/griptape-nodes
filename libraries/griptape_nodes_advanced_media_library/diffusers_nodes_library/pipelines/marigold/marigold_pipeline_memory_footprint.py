...
def print_marigold_pipeline_memory_footprint(pipe: diffusers.MarigoldDepthPipeline) -> None:
...
@cache
def optimize_marigold_pipeline_memory_footprint(pipe: diffusers.MarigoldDepthPipeline) -> None:
    """Optimize pipeline memory footprint."""
    device = get_best_device()

    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for Marigold pipeline optimization")
    ...
    print_marigold_pipeline_memory_footprint(pipe)