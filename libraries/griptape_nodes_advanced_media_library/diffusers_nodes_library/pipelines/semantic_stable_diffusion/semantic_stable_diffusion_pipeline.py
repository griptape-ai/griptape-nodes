...
from diffusers_nodes_library.pipelines.semantic_stable_diffusion.semantic_stable_diffusion_pipeline_memory_footprint import (
    optimize_semantic_stable_diffusion_pipeline_memory_footprint,
)  # type: ignore[reportMissingImports]
from diffusers_nodes_library.pipelines.semantic_stable_diffusion.semantic_stable_diffusion_pipeline_parameters import (
    SemanticStableDiffusionPipelineParameters,  # type: ignore[reportMissingImports]
)
...
class SemanticStableDiffusionPipeline(ControlNode):
    """Griptape wrapper around diffusers.pipelines.semantic_stable_diffusion.SemanticStableDiffusionPipeline."""

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = SemanticStableDiffusionPipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        with self.log_params.append_profile_to_logs("Loading model metadata"):
            ...
            pipe = model_cache.from_pretrained(
                diffusers.SemanticStableDiffusionPipeline,
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_semantic_stable_diffusion_pipeline_memory_footprint(pipe)

        num_inference_steps = self.pipe_params.get_num_inference_steps()

        def callback_on_step_end(
            pipe: diffusers.SemanticStableDiffusionPipeline,
            i: int,
            _t: Any,
