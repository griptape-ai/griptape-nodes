...
class StableDiffusionDiffeditPipeline(ControlNode):
    ...
    def _process(self) -> AsyncResult | None:
        ...
        mask_image = self.pipe_params.get_mask_image()
        if mask_image is None:
            self.log_params.append_to_logs("Generating mask...\n")
            with self.log_params.append_profile_to_logs("Mask generation"):
        ...
        self.log_params.append_to_logs("Performing DDIM inversion...\n")
        with self.log_params.append_profile_to_logs("DDIM inversion"):
            inv_latents = pipe.invert(**self.pipe_params.get_inversion_kwargs()).latents
        ...
        output_image_pil = pipe(
            **self.pipe_params.get_pipe_kwargs(mask_image, inv_latents),
            output_type="pil",
