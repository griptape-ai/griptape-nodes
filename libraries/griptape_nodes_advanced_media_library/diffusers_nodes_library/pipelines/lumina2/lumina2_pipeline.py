...
from diffusers_nodes_library.pipelines.lumina2.lumina2_pipeline_memory_footprint import (
    optimize_lumina2_pipeline_memory_footprint,
)  # type: ignore[reportMissingImports]
from diffusers_nodes_library.pipelines.lumina2.lumina2_pipeline_parameters import (
    Lumina2PipelineParameters,  # type: ignore[reportMissingImports]
)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        with self.log_params.append_profile_to_logs("Loading model metadata"):
            ...
            pipe = model_cache.from_pretrained(
                diffusers.Lumina2Text2ImgPipeline,
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_lumina2_pipeline_memory_footprint(pipe)

        num_inference_steps = self.pipe_params.get_num_inference_steps()

        def callback_on_step_end(
            pipe: diffusers.Lumina2Text2ImgPipeline,
            i: int,
            _t: Any,
