...
class HidreamImagePipeline(ControlNode):
    """Griptape wrapper around diffusers.pipelines.hidream_image.HidreamImagePipeline."""

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = HidreamImagePipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_hidream_image_pipeline_memory_footprint(pipe)
