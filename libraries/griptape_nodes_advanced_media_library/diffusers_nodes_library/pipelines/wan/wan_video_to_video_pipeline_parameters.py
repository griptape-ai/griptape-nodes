...
class WanVideoToVideoPipelineParameters:
    ...
    def add_input_parameters(self) -> None:
        ...
        self._node.add_parameter(
            Parameter(
                ...
                input_types=["VideoUrlArtifact"],
                type="VideoUrlArtifact",
                tooltip="Mask video defining conditioning regions (optional)",
            )
        )
        self._node.add_parameter(
            Parameter(
        ...
        self._node.add_parameter(
            Parameter(
                ...
                input_types=["float"],
                type="float",
                tooltip="Conditioning scale for video input (higher = more input video influence)",
            )
        )
    ...
    def get_conditioning_scale(self) -> float:
        return float(self._node.get_parameter_value("conditioning_scale"))
    ...
    def get_pipe_kwargs(self) -> dict:
        pipe_kwargs = {
            ...
            "conditioning_scale": self.get_conditioning_scale(),
            "generator": self._seed_parameter.get_generator(),
            "output_type": "pil",
        }

        # Add input video if provided
        input_video = self.get_input_video()
        if input_video is not None:
            pipe_kwargs["video"] = self._video_artifact_to_pil_frames(input_video)

        # Add mask video if provided
        mask_video = self.get_mask_video()
        if mask_video is not None:
            pipe_kwargs["mask"] = self._video_artifact_to_pil_frames(mask_video)
