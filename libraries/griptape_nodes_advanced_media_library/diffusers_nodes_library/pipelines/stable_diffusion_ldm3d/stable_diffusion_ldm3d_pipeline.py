...
from diffusers_nodes_library.pipelines.stable_diffusion_ldm3d.stable_diffusion_ldm3d_pipeline_memory_footprint import (
    optimize_stable_diffusion_ldm3d_pipeline_memory_footprint,
)  # type: ignore[reportMissingImports]
from diffusers_nodes_library.pipelines.stable_diffusion_ldm3d.stable_diffusion_ldm3d_pipeline_parameters import (
    StableDiffusionLdm3dPipelineParameters,  # type: ignore[reportMissingImports]
)
...
class StableDiffusionLdm3dPipeline(ControlNode):
    """Griptape wrapper around diffusers.StableDiffusionLDM3DPipeline."""

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = StableDiffusionLdm3dPipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        self.log_params.append_to_logs("Preparing Stable Diffusion LDM3D models...\n")
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_stable_diffusion_ldm3d_pipeline_memory_footprint(pipe)

        num_inference_steps = self.pipe_params.get_num_inference_steps()

        def callback_on_step_end(
            pipe: diffusers.StableDiffusionLDM3DPipeline,
            i: int,
            _t: Any,
