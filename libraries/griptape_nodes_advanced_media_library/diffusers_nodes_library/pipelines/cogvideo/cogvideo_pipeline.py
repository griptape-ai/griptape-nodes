...
from diffusers_nodes_library.pipelines.cogvideo.cogvideo_pipeline_memory_footprint import (
    optimize_cogvideo_pipeline_memory_footprint,
)  # type: ignore[reportMissingImports]
from diffusers_nodes_library.pipelines.cogvideo.cogvideo_pipeline_parameters import (
    CogvideoPipelineParameters,  # type: ignore[reportMissingImports]
)
...
class CogvideoPipeline(ControlNode):
    """Griptape wrapper around diffusers.pipelines.cogvideo.CogVideoXPipeline."""
    
    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = CogvideoPipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        self.preprocess()
        self.log_params.append_to_logs("Preparing CogVideo models...\n")
        ...
        self.log_params.append_to_logs(f"Generating {num_frames} frame video with {num_inference_steps} inference steps...\n")
        ...
        output_video = result.frames[0]
        self.pipe_params.publish_output_video(output_video)
        self.log_params.append_to_logs("Video generation complete.\n")