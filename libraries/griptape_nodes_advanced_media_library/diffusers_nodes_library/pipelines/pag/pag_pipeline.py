...
from diffusers_nodes_library.pipelines.pag.pag_pipeline_memory_footprint import (  # type: ignore[reportMissingImports]
    optimize_pag_pipeline_memory_footprint,
    print_pag_pipeline_memory_footprint,
)
from diffusers_nodes_library.pipelines.pag.pag_pipeline_parameters import (  # type: ignore[reportMissingImports]
    PagPipelineParameters,
)
...
class PagPipeline(ControlNode):
    """Griptape wrapper around diffusers.pipelines.pag.StableDiffusionXLPAGPipeline."""

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = PagPipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_pag_pipeline_memory_footprint(pipe)

        num_inference_steps = self.pipe_params.get_num_inference_steps()

        def callback_on_step_end(
            pipe: diffusers.StableDiffusionXLPAGPipeline,
            i: int,
            _t: Any,
