...
class AllegroPipelineParameters:
    ...
    def publish_output_video_preview_latents(self, pipe: diffusers.AllegroPipeline, latents: Any) -> None:
        ...
        Only generates previews if 30 seconds have elapsed since the last preview completed.
        """
        current_time = time.time()

        # Check if enough time has passed since last preview
        if current_time - self._last_preview_time < self._preview_throttle_seconds:
            logger.debug(
                "Skipping preview generation - only %.1f seconds elapsed since last preview (need %.1f)",
                current_time - self._last_preview_time,
                self._preview_throttle_seconds,
            )
            return
        ...
        try:
            ...
            # Update last preview time after successful generation
