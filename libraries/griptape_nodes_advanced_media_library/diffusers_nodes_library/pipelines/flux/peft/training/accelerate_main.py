...
class FluxLoraTrainingScript:
    ...
    def _configure_device(self) -> None:
        assert self.accelerator is not None, "Accelerator must be configured before configuring device and dtype."
    ...
    def _load_schedulers(self) -> None:
        assert self.frozen_dtype is not None, "Frozen dtype must be configured before loading schedulers."
    ...
    def _configure_gradient_checkpointing(self) -> None:
        assert self.transformer is not None, "Transformer must be loaded before configuring gradient checkpointing."
    ...
    def _configure_lora_adapter(self) -> None:
        assert self.transformer is not None, "Transformer must be loaded before configuring LoRA adapter."
    ...
    def _log_trainable_parameters(self) -> None:
        assert self.transformer is not None, "Transformer must be loaded before logging trainable parameters."
    ...
    def _register_save_load_hooks_for_transformer(self) -> None:
        assert self.accelerator is not None, "Accelerator must be configured before registering save/load hooks."
        assert self.transformer is not None, "Transformer must be loaded before registering save/load hooks."
    ...
    def _scale_learning_rate(self) -> None:
        assert self.accelerator is not None, "Accelerator must be configured before scaling learning rate."
        assert self.transformer is not None, "Transformer must be loaded before scaling learning rate."

    def _configure_optimizer(self) -> None:
        assert self.accelerator is not None, "Accelerator must be configured before configuring optimizer."
        assert self.transformer is not None, "Transformer must be loaded before configuring optimizer."
    ...
    def _create_dataloader(self) -> None:
        assert self.train_dataset is not None, "Train dataset must be loaded before creating dataloader."
    ...
    def _compute_text_embeddings(self, prompt: str | list[str]) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        assert self.device is not None, "Device must be configured before computing text embeddings."
        with torch.no_grad():
    ...
    def _cache_text_embedding(self) -> None:
        ...
        assert self.text_encoder_one is not None, "Text encoder one must be loaded before caching text embeddings."
        assert self.text_encoder_two is not None, "Text encoder two must be loaded before caching text embeddings."
    ...
    def _cache_latents(self) -> None:
        if args.cache_latents:
            self._load_vae()
            assert self.vae is not None, "VAE must be loaded before caching latents."
    ...
    def _determine_number_scheduler_steps(self) -> None:
        assert self.accelerator is not None, (
            "Accelerator must be configured before determining number of scheduler steps."
        )
    ...
    def _configure_scheduler(self) -> None:
        assert self.optimizer is not None, "Optimizer must be configured before configuring scheduler."
        assert self.num_training_steps_for_scheduler is not None, (
            "Number of training steps must be determined before configuring scheduler."
        )
    ...
    def _prepare_for_accelerator(self) -> None:
        assert self.accelerator is not None, "Accelerator must be configured before preparing for accelerator."
        assert self.transformer is not None, "Transformer must be loaded before preparing for accelerator."
        assert self.optimizer is not None, "Optimizer must be configured before preparing for accelerator."
        assert self.train_dataloader is not None, "Train dataloader must be created before preparing for accelerator."
        assert self.lr_scheduler is not None, (
            "Learning rate scheduler must be configured before preparing for accelerator."
        )
    ...
    def _initialize_trackers(self) -> None:
        assert self.accelerator is not None, "Accelerator must be configured before initializing trackers."
    ...
    def _log_training_start_summary(self) -> None:
        assert self.train_dataset is not None, "Train dataset must be loaded before logging training start summary."
        assert self.train_dataloader is not None, (
            "Train dataloader must be created before logging training start summary."
        )
    ...
    def _load_checkpoint_or_default(self) -> tuple[int, int, int]:
        assert self.accelerator is not None, "Accelerator must be configured before trying to load checkpoint."
    ...
    def _init_steps_progress_bar(self, initial_global_step: int) -> tqdm:
        assert self.accelerator is not None, "Accelerator must be configured before starting training."
    ...
    def _get_sigmas(self, timesteps: torch.Tensor, n_dim: int = 4, dtype: torch.dtype = torch.float32) -> torch.Tensor:
        assert self.accelerator is not None, "Accelerator must be configured before getting sigmas."
        assert self.noise_scheduler_copy is not None, "Noise scheduler copy must be created before getting sigmas."
    ...
    def _train(self) -> None:
        ...
        for epoch in range(first_epoch, args.num_train_epochs):
            ...
            for step, batch in enumerate(self.train_dataloader):
                logger.info("Starting step %d/%d", step + 1, len(self.train_dataloader))
    ...
    def _step(self, step: int, batch: dict) -> Any:  # noqa: PLR0915
        assert self.accelerator is not None, "Accelerator must be configured before stepping."
        assert self.transformer is not None, "Transformer must be loaded before stepping."
        assert self.optimizer is not None, "Optimizer must be configured before stepping."
        assert self.train_dataloader is not None, "Train dataloader must be created before stepping."
        assert self.lr_scheduler is not None, "Learning rate scheduler must be configured before stepping."
        assert self.vae_config_shift_factor is not None, "VAE config shift factor must be set before stepping."
        assert self.vae_config_scaling_factor is not None, "VAE config scaling factor must be set before stepping."
        assert self.vae_config_block_out_channels is not None, (
            "VAE config block out channels must be set before stepping."
        )
        ...
        with self.accelerator.accumulate([self.transformer]):
            ...
            if args.cache_latents:
                ...
                assert self.vae is not None, "VAE must be loaded before stepping if caching latents is disabled."
    ...
    def _unwrap_model(self, model: torch.nn.Module) -> torch.nn.Module:
        assert self.accelerator is not None, "Accelerator must be configured before unwrapping model."
        return unwrap_model(self.accelerator, model)

    def _save_checkpoint(self, global_step: int) -> None:
        assert self.accelerator is not None, "Accelerator must be configured before saving checkpoint."
    ...
    def _publish_validation_images(self, phase: str, epoch: int | None) -> list[Image]:
        assert self.accelerator is not None, "Accelerator must be configured before logging validation."
        assert self.text_encoder_one is not None, "Text encoder one must be loaded before logging validation."
        assert self.text_encoder_two is not None, "Text encoder two must be loaded before logging validation."
    ...
    def _save_lora_weights(self) -> None:
        assert self.accelerator is not None, "Accelerator must be configured before saving LoRA weights."
