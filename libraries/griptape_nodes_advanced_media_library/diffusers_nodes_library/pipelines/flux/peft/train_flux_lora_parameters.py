...
class TrainFluxLoraParameters:
    ...
    def after_value_set(self, parameter: Parameter, value: Any, modified_parameters_set: set[str]) -> None:  # noqa: ARG002
        if parameter.name in {"gradient_accumulation_steps", "train_batch_size"}:
            ...
            effective_batch_size = self.get_train_batch_size() * self.get_gradient_accumulation_steps()
        ...
        if parameter.name in {"num_train_epochs", "repeats", "effective_batch_size", "max_train_steps"}:
            training_data_dir = self.get_training_data_directory()
            if not training_data_dir or not Path(training_data_dir).exists():
                ...
                training_data_size = len(list(Path(self.get_training_data_directory()).glob("*.txt")))
                effective_train_steps = min(
                    self.get_num_train_epochs()
                    * (self.get_repeats() * training_data_size / self.get_train_batch_size()),
                    self.get_max_train_steps(),
                )
