...
from diffusers_nodes_library.common.utils.parameter_utils import (  # type: ignore[reportMissingImports]
    FluxLoraRepoRevisionParameters,  # type: ignore[reportMissingImports]
...
class HuggingFaceLoraNode(ControlNode):
    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.category = "image"
        self.description = self.__class__.__name__
        self.flux_params = FluxPipelineParameters(self)
        self.lora_revisions_params = FluxLoraRepoRevisionParameters(self, self.get_repo_id())
