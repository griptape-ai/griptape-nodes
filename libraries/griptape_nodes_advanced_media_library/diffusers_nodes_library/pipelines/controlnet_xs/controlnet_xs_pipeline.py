...
from diffusers_nodes_library.pipelines.controlnet_xs.controlnet_xs_pipeline_memory_footprint import (
    optimize_controlnet_xs_pipeline_memory_footprint,
)  # type: ignore[reportMissingImports]
from diffusers_nodes_library.pipelines.controlnet_xs.controlnet_xs_pipeline_parameters import (
    ControlnetXsPipelineParameters,  # type: ignore[reportMissingImports]
)
...
class ControlnetXsPipeline(ControlNode):
    """Griptape wrapper around diffusers.StableDiffusionControlNetXSPipeline."""

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = ControlnetXsPipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        self.log_params.append_to_logs("Preparing Stable Diffusion ControlNet-XS models...\n")

        with self.log_params.append_profile_to_logs("Loading controlnet model metadata"):
        ...
        with self.log_params.append_profile_to_logs("Loading model metadata"):
            ...
            pipe = model_cache.from_pretrained(
                diffusers.StableDiffusionControlNetXSPipeline,
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_controlnet_xs_pipeline_memory_footprint(pipe)

        num_inference_steps = self.pipe_params.get_num_inference_steps()

        def callback_on_step_end(
            pipe: diffusers.StableDiffusionControlNetXSPipeline,
            i: int,
            _t: Any,
