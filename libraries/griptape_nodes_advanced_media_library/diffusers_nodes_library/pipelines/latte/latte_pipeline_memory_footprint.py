...
def print_latte_pipeline_memory_footprint(
    pipe: diffusers.LattePipeline,  # type: ignore[reportMissingImports]
) -> None:
    """Print memory footprint for the main sub-modules of Latte pipelines."""
...
@cache
def optimize_latte_pipeline_memory_footprint(
    pipe: diffusers.LattePipeline,  # type: ignore[reportMissingImports]
) -> None:
    """Apply a set of heuristics to minimise VRAM / RAM footprint at inference time.

    The logic follows the same rationale as the optimization helper for other
    pipelines but adapted to the component layout of Latte.
    """
    device = get_best_device()

    if not torch.cuda.is_available():
        msg = "CUDA is required for Latte pipeline optimization"
