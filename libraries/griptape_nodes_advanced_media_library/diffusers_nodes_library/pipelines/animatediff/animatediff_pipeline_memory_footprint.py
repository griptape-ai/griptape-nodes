...
ANIMATEDIFF_PIPELINE_COMPONENT_NAMES = [
    "vae",
    "text_encoder",
    "unet",
    "motion_adapter",
...
def print_animatediff_pipeline_memory_footprint(
    pipe: diffusers.AnimateDiffPipeline,  # type: ignore[reportMissingImports]
) -> None:
    """Print memory footprint for the main sub-modules of AnimateDiff pipelines."""
    print_pipeline_memory_footprint(pipe, ANIMATEDIFF_PIPELINE_COMPONENT_NAMES)
...
def optimize_animatediff_pipeline_memory_footprint(
    ...
    model_memory = get_total_memory_footprint(pipe, ANIMATEDIFF_PIPELINE_COMPONENT_NAMES)
    ...
    if device.type == "cuda":
        ...
        logger.info("Require memory for AnimateDiff Pipeline: %s", to_human_readable_size(model_memory))
