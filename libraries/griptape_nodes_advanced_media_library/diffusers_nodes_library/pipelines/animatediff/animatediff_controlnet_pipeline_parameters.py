...
class AnimateDiffControlNetPipelineParameters:
    ...
    def add_input_parameters(self) -> None:
        ...
        self._node.add_parameter(
            Parameter(
                ...
                tooltip="Control video for ControlNet guidance (conditioning frames)",
            )
        )
        self._node.add_parameter(
            Parameter(
    ...
    def validate_before_node_run(self) -> list[Exception] | None:
        ...
        # Validate control video is provided
        control_video = self.get_control_video()
        if control_video is None:
            errors.append(ValueError("Control video is required for AnimateDiff ControlNet pipeline"))
    ...
    def get_control_video(self) -> VideoUrlArtifact | None:
        return self._node.get_parameter_value("control_video")

    def get_control_video_pil_frames(self) -> list[Image] | None:
        """Get control video frames as a list of PIL Images."""
        control_video = self.get_control_video()
        if control_video is None:
            return None
        return self._video_artifact_to_pil_frames(control_video)

    def _video_artifact_to_pil_frames(self, video_artifact: VideoUrlArtifact) -> list[Image]:
    ...
    def get_pipe_kwargs(self) -> dict:
        return {
            ...
            "image": self.get_control_video_pil_frames(),
