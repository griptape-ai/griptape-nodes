...
def print_visualcloze_pipeline_memory_footprint(pipe: diffusers.VisualClozePipeline) -> None:
...
@cache
def optimize_visualcloze_pipeline_memory_footprint(pipe: diffusers.VisualClozePipeline) -> None:
    """Optimize pipeline memory footprint."""
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is not available. VisualCloze requires GPU acceleration.")
    ...
    print_visualcloze_pipeline_memory_footprint(pipe)
    ...
    if device == torch.device("cuda"):
        # For VisualCloze, we use sequential cpu offload
