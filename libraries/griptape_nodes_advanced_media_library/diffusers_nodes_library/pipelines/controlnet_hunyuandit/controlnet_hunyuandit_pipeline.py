...
from diffusers_nodes_library.pipelines.controlnet_hunyuandit.controlnet_hunyuandit_pipeline_memory_footprint import (
    optimize_controlnet_hunyuandit_pipeline_memory_footprint,
)  # type: ignore[reportMissingImports]
from diffusers_nodes_library.pipelines.controlnet_hunyuandit.controlnet_hunyuandit_pipeline_parameters import (
    ControlnetHunyuanditPipelineParameters,  # type: ignore[reportMissingImports]
)
...
class ControlnetHunyuanditPipeline(ControlNode):
    """Griptape wrapper around diffusers.pipelines.controlnet_hunyuandit.HunyuanDiTControlNetPipeline."""

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = ControlnetHunyuanditPipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_controlnet_hunyuandit_pipeline_memory_footprint(pipe)

        num_inference_steps = self.pipe_params.get_num_inference_steps()

        def callback_on_step_end(
            pipe: diffusers.HunyuanDiTControlNetPipeline,
            i: int,
            _t: Any,
