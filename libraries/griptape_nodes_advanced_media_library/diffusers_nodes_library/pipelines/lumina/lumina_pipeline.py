...
from diffusers_nodes_library.pipelines.lumina.lumina_pipeline_memory_footprint import (
    optimize_lumina_pipeline_memory_footprint,
)  # type: ignore[reportMissingImports]
from diffusers_nodes_library.pipelines.lumina.lumina_pipeline_parameters import (
    LuminaPipelineParameters,  # type: ignore[reportMissingImports]
)
...
class LuminaPipeline(ControlNode):
    """Griptape wrapper around diffusers.pipelines.lumina.LuminaText2ImgPipeline."""
    
    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = LuminaPipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        with self.log_params.append_profile_to_logs("Loading model metadata"):
            ...
            pipe = model_cache.from_pretrained(
                diffusers.LuminaText2ImgPipeline,
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_lumina_pipeline_memory_footprint(pipe)

        num_inference_steps = self.pipe_params.get_num_inference_steps()

        def callback_on_step_end(
            pipe: diffusers.LuminaText2ImgPipeline,
            i: int,
            _t: Any,
