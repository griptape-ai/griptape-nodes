...
from diffusers_nodes_library.pipelines.controlnet_sd3.controlnet_sd3_pipeline_memory_footprint import (
    optimize_controlnet_sd3_pipeline_memory_footprint,
)  # type: ignore[reportMissingImports]
from diffusers_nodes_library.pipelines.controlnet_sd3.controlnet_sd3_pipeline_parameters import (
    ControlnetSd3PipelineParameters,  # type: ignore[reportMissingImports]
)
...
class ControlnetSd3Pipeline(ControlNode):
    """Griptape wrapper around diffusers.StableDiffusion3ControlNetPipeline."""

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = ControlnetSd3PipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        self.log_params.append_to_logs("Preparing Stable Diffusion 3 ControlNet models...\n")
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_controlnet_sd3_pipeline_memory_footprint(pipe)

        num_inference_steps = self.pipe_params.get_num_inference_steps()

        def callback_on_step_end(
            pipe: diffusers.StableDiffusion3ControlNetPipeline,
            i: int,
            _t: Any,
