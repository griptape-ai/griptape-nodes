...
from diffusers_nodes_library.pipelines.stable_diffusion_sag.stable_diffusion_sag_pipeline_memory_footprint import (
    optimize_stable_diffusion_sag_pipeline_memory_footprint,
)  # type: ignore[reportMissingImports]
from diffusers_nodes_library.pipelines.stable_diffusion_sag.stable_diffusion_sag_pipeline_parameters import (
    StableDiffusionSagPipelineParameters,  # type: ignore[reportMissingImports]
)
...
class StableDiffusionSagPipeline(ControlNode):
    """Griptape wrapper around diffusers.StableDiffusionSAGPipeline."""

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = StableDiffusionSagPipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        self.log_params.append_to_logs("Preparing Stable Diffusion SAG models...\n")
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_stable_diffusion_sag_pipeline_memory_footprint(pipe)

        num_inference_steps = self.pipe_params.get_num_inference_steps()

        def callback_on_step_end(
            pipe: diffusers.StableDiffusionSAGPipeline,
            i: int,
            _t: Any,
