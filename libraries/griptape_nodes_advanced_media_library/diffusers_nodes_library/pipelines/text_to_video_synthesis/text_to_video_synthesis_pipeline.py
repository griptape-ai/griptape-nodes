...
from diffusers_nodes_library.pipelines.text_to_video_synthesis.text_to_video_synthesis_pipeline_parameters import (  # type: ignore[reportMissingImports]
    TextToVideoSynthesisPipelineParameters,
)
from diffusers_nodes_library.pipelines.text_to_video_synthesis.text_to_video_synthesis_pipeline_memory_footprint import (  # type: ignore[reportMissingImports]
    optimize_text_to_video_synthesis_pipeline_memory_footprint,
    print_text_to_video_synthesis_pipeline_memory_footprint,
)
...
class TextToVideoSynthesisPipeline(ControlNode):
    """Griptape wrapper around diffusers.pipelines.text_to_video_synthesis.TextToVideoSDPipeline."""

    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
        self.pipe_params = TextToVideoSynthesisPipelineParameters(self)
    ...
    def _process(self) -> AsyncResult | None:
        ...
        with self.log_params.append_profile_to_logs("Loading model"), self.log_params.append_logs_to_logs(logger):
            optimize_text_to_video_synthesis_pipeline_memory_footprint(pipe)

        num_inference_steps = self.pipe_params.get_num_inference_steps()

        def callback_on_step_end(
            pipe: diffusers.TextToVideoSDPipeline,
            i: int,
            _t: Any,
        ...
        self.pipe_params.publish_output_video(output_frames)
