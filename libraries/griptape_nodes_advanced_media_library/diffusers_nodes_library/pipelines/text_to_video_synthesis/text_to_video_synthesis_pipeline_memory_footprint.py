...
def print_text_to_video_synthesis_pipeline_memory_footprint(pipe: diffusers.TextToVideoSDPipeline) -> None:
...
@cache
def optimize_text_to_video_synthesis_pipeline_memory_footprint(pipe: diffusers.TextToVideoSDPipeline) -> None:
    """Optimize pipeline memory footprint."""
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is not available. Text-to-video synthesis requires GPU acceleration.")
    ...
    print_text_to_video_synthesis_pipeline_memory_footprint(pipe)
    ...
    if device == torch.device("cuda"):
        # For text-to-video synthesis, we use sequential cpu offload
