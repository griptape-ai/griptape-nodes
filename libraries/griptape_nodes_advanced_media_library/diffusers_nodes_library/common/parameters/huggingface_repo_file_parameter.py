...
class HuggingFaceRepoFileParameter:
    ...
    def __init__(self, node: ControlNode, repo_files: list[tuple[str, str]], parameter_name: str = "model"):
        self._node = node
        self._parameter_name = parameter_name
        self._repo_files = repo_files
        self._repo_revisions = [
            repo_revision
            for (repo, file) in repo_files
            for repo_revision in list_repo_revisions_with_file_in_cache(repo, file)
        ]

    def add_message_parameters(self) -> None:
