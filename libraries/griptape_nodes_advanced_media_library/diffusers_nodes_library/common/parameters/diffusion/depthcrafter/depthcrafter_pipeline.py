...
from diffusers.pipelines.stable_video_diffusion.pipeline_stable_video_diffusion import (  # pyright: ignore[reportMissingImports]
    StableVideoDiffusionPipelineOutput,
    _resize_with_antialiasing,
    retrieve_timesteps,
)
from diffusers.utils.torch_utils import randn_tensor  # pyright: ignore[reportMissingImports]
...
class DepthCrafterVideoDiffusionPipeline(diffusers.StableVideoDiffusionPipeline):
    ...
    @classmethod
    def from_single_file(cls, *args, **kwargs) -> Self:
    ...
    def encode_video(
        ...
        """Encode video frames to video embeddings.
    ...
    @staticmethod
    def check_inputs(video: Any, height: Any, width: Any) -> None:
        ...
        if not isinstance(video, torch.Tensor) and not isinstance(video, np.ndarray):
            msg = f"Expected `video` to be a `torch.Tensor` or `VideoReader`, but got a {type(video)}"
    ...
    @torch.no_grad()
    def __call__(  # noqa: ANN204, C901, PLR0912, PLR0913, PLR0915
        self,
        ...
        callback_on_step_end_tensor_inputs: list[str] = list("latents"),  # noqa: B006
        return_dict: bool = True,  # noqa: FBT001, FBT002
        overlap: int = 25,
        track_time: bool = False,  # noqa: FBT001, FBT002
        ...
        if track_time:
            ...
            assert start_event is not None  # noqa: S101
            encode_event.record()
            torch.cuda.synchronize()
            elapsed_time_ms = start_event.elapsed_time(encode_event)
            msg = f"Elapsed time for encoding video: {elapsed_time_ms} ms"
            logger.info(msg)
        ...
        while idx_start < num_frames - overlap:
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    if latents_all is not None and i == 0:
                        ...
                        assert self.scheduler.sigmas is not None  # noqa: S101
                        latents[:, :overlap] = (  # pyright: ignore[reportOptionalSubscript]
                            latents_all[:, -overlap:]  # pyright: ignore[reportOptionalSubscript]
                            + latents[:, :overlap] / self.scheduler.init_noise_sigma * self.scheduler.sigmas[i]  # pyright: ignore[reportOptionalSubscript]
                        )

                    latent_model_input = latents  # [1, t, c, h, w]
                    latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)  # [1, t, c, h, w]
                    latent_model_input = torch.cat([latent_model_input, video_latents_current], dim=2)
                    ...
                    if callback_on_step_end is not None:
                        ...
                        callback_outputs = callback_on_step_end(self, i, t, callback_kwargs)  # pyright: ignore[reportCallIssue]

                        latents = callback_outputs.pop("latents", latents)  # pyright: ignore[reportAttributeAccessIssue]
            ...
            if latents_all is None:
                latents_all = latents.clone()  # pyright: ignore[reportOptionalMemberAccess]
            else:
                # Type narrowing for latents_all and weights in else block
                assert latents_all is not None  # noqa: S101
                assert weights is not None  # noqa: S101
                latents_all[:, -overlap:] = latents[:, :overlap] * weights + latents_all[:, -overlap:] * (1 - weights)  # pyright: ignore[reportOptionalSubscript]
                latents_all = torch.cat([latents_all, latents[:, overlap:]], dim=1)  # pyright: ignore[reportOptionalSubscript]
        ...
        if output_type != "latent":
            ...
            if track_time:
                assert decode_event is not None  # noqa: S101
                assert denoise_event is not None  # noqa: S101
                decode_event.record()
                torch.cuda.synchronize()
