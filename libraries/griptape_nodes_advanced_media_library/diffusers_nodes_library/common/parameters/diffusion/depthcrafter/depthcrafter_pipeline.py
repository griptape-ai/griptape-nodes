...
class DepthCrafterVideoDiffusionPipeline(diffusers.StableVideoDiffusionPipeline):
    ...
    Args:
        diffusers (_type_): _description_
    ...
    def encode_vae_video(
        ...
        for i in range(0, video.shape[0], chunk_size):
            video_latents.append(self.vae.encode(video[i : i + chunk_size]).latent_dist.mode())
    ...
    def __call__(  # noqa: ANN204, C901, PLR0912, PLR0913, PLR0915
        ...
        if track_time:
            encode_event.record()  # pyright: ignore[reportPossiblyUnboundVariable]
            torch.cuda.synchronize()
            elapsed_time_ms = start_event.elapsed_time(encode_event)  # pyright: ignore[reportPossiblyUnboundVariable]
