...
class WanTextToVideoGeneration(SuccessFailureNode):
    ...
    Inputs:
        ...
        - audio_url (str): Optional URL to audio file (only for wan2.5-t2v-preview)
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        # Audio URL parameter (only for wan2.5-t2v-preview)
        self.add_parameter(
            Parameter(
                name="audio_url",
                input_types=["str"],
                type="str",
                default_value="",
                tooltip="Optional URL to audio file (only supported by wan2.5-t2v-preview)",
                allowed_modes={ParameterMode.INPUT, ParameterMode.PROPERTY},
                ui_options={
    ...
    async def _process(self) -> None:
        ...
        model = params["model"]
        self._log(f"Generating video with {model}")
    ...
    def _get_parameters(self) -> dict[str, Any]:
        ...
        # Validate audio URL
        if audio_url and not model_config["supports_audio"]:
            msg = f"{model} does not support audio. Only wan2.5-t2v-preview supports audio_url parameter."
            raise ValueError(msg)
    ...
    def _build_payload(self, params: dict[str, Any]) -> dict[str, Any]:
        # Convert size display name to resolution format
        size_resolution = SIZE_RESOLUTIONS.get(params["size"], "848x480")
        ...
        # Add audio if provided (only for wan2.5-t2v-preview)
        if params["audio_url"]:
            payload["parameters"]["audio"] = params["audio_url"]
