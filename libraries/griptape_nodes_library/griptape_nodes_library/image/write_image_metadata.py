...
class WriteImageMetadataNode(SuccessFailureNode):
    """Write custom key-value metadata to PNG images.

    Writes metadata to PNG text chunks.
    ...
    def process(self) -> None:  # noqa: C901, PLR0911, PLR0915
        ...
        if pil_image.format != "PNG":
            error_msg = f"{self.name}: Only PNG format supported, got {pil_image.format}"
            logger.warning(error_msg)
            self._set_status_results(was_successful=False, result_details=error_msg)
            self._handle_failure_exception(ValueError(error_msg))
            return

        # Write metadata to PNG chunks
        try:
            image_bytes = self._write_png_metadata(pil_image, metadata_dict)
        except Exception as e:
        ...
        # Success
        success_msg = f"Successfully wrote {len(metadata_dict)} metadata entries"
        self._set_status_results(was_successful=True, result_details=success_msg)
        logger.info(f"{self.name}: {success_msg}")

    def _write_png_metadata(self, pil_image: Image.Image, metadata: dict[str, str]) -> bytes:
        """Write metadata to PNG text chunks.

        Creates PngInfo with existing text chunks and adds new metadata.

        Args:
            pil_image: PIL Image to write metadata to
            metadata: Dictionary of key-value pairs to write

        Returns:
            Image bytes with metadata written
        """
