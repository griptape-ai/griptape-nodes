...
class QwenImageEdit(SuccessFailureNode):
    ...
    async def _process(self) -> None:
        ...
        model = params["model"]
        self._log(f"Editing images with {model}")
        ...
        try:
            ...
            if not generation_id:
                ...
                self._set_status_results(
                    ...
                    result_details="No generation_id returned from API. Cannot proceed with editing.",
                )
                return
    ...
    def _log_request(self, payload: dict[str, Any]) -> None:
        with suppress(Exception):
            ...
            if "input" in sanitized_payload and "messages" in sanitized_payload["input"]:
                for msg in sanitized_payload["input"]["messages"]:
                    if "content" in msg:
    ...
    async def _poll_for_result(self, generation_id: str, headers: dict[str, str]) -> None:
        ...
        async with httpx.AsyncClient() as client:
            for attempt in range(max_attempts):
                try:
                    ...
                    if status in [STATUS_FAILED, STATUS_ERROR, STATUS_REQUEST_MODERATED, STATUS_CONTENT_MODERATED]:
                        self._log(f"Editing failed with status: {status}")
                        self._set_safe_defaults()
                    ...
                    if attempt == max_attempts - 1:
                        self._set_safe_defaults()
                        error_msg = f"Failed to poll editing status: HTTP {e.response.status_code}"
                    ...
                    if attempt == max_attempts - 1:
                        self._set_safe_defaults()
                        error_msg = f"Failed to poll editing status: {e}"
                        self._set_status_results(was_successful=False, result_details=error_msg)
                        return
            ...
            self._set_status_results(
                was_successful=False,
                result_details=f"Image editing timed out after {max_attempts * poll_interval} seconds waiting for result.",
            )

    async def _handle_success(self, response: dict[str, Any], image_url: str) -> None:
        """Handle successful editing result."""
