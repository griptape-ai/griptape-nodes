...
class SeedreamImageGeneration(GriptapeProxyNode):
    ...
    async def _save_single_image_from_url(
        ...
        try:
            ...
            # Attempt to map the absolute path back to a portable macro form (e.g., "{outputs}/image.png")
            artifact_value = str(actual_path)
            map_result = GriptapeNodes.handle_request(AttemptMapAbsolutePathToProjectRequest(absolute_path=actual_path))
            if (
                isinstance(map_result, AttemptMapAbsolutePathToProjectResultSuccess)
                and map_result.mapped_path is not None
            ):
                artifact_value = map_result.mapped_path

            return ImageUrlArtifact(value=artifact_value, name=actual_path.name)
