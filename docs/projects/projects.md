...
| `file_extension_macros`           | No       | Extension-to-folder routing; see [File Extension Macros](file_extension_macros.md) |
...
- `file_extension_macros` entries merge per-key the same way as environment entries. A `null` value tombstones the base entry.
