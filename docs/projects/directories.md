...
| ---------- | ---------------------------------------- | ------------------------------------------------------------- |
| `inputs`   | `{project_dir}/inputs`                   | Files brought into the project (uploaded, copied, downloaded) |
| `outputs`  | `{project_dir}/outputs`                  | Files generated by nodes                                      |
| `temp`     | `{project_dir}/temp`                     | Temporary working files                                       |
| `previews` | `{project_dir}/.griptape-nodes-previews` | Preview thumbnails (hidden folder)                            |

All default paths use `{project_dir}` and resolve to absolute paths inside your project directory.
...
Template:  {outputs}/{file_name_base}.{file_extension}
           ↓
Resolved:  /your/project/outputs/my_image.png
```
...
directories:
  outputs:
    path_macro: "{project_dir}/renders/final"   # explicit project-relative path
    # path_macro: "renders/final"               # relative paths also work (resolved against project dir)
