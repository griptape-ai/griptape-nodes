...
| ---------- | ------------------------------------------ | ------------------------------------------------------------- |
| `inputs`   | `{workspace_dir}/inputs`                   | Files brought into the project (uploaded, copied, downloaded) |
| `outputs`  | `{workspace_dir}/outputs`                  | Files generated by nodes                                      |
| `temp`     | `{workspace_dir}/temp`                     | Temporary working files                                       |
| `previews` | `{workspace_dir}/.griptape-nodes-previews` | Preview thumbnails (hidden folder)                            |

All default paths use `{workspace_dir}` and resolve to absolute paths inside your project's workspace directory.
...
directories:
  outputs:
    path_macro: "{workspace_dir}/renders/final"  # explicit workspace-relative path
    # path_macro: "renders/final"                # relative paths also work (resolved against workspace dir)
