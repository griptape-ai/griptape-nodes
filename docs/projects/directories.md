...
All default paths are relative and resolve to absolute paths inside your project's workspace directory. Relative paths are resolved against `{workspace_dir}`, which defaults to `{project_dir}`.
...
Template:  {outputs}/{file_name_base}.{file_extension}
           ↓
Resolved:  /your/workspace/outputs/my_image.png
```
...
directories:
  outputs:
    path_macro: "renders/final"                  # relative path (resolved against workspace dir)
    # path_macro: "{workspace_dir}/renders/final" # explicit workspace-relative path (same result)
    # path_macro: "~/Downloads"                   # ~ is expanded to your home directory
    # path_macro: "/absolute/path/to/renders"     # absolute paths are used as-is
```

Now `{outputs}` in any macro resolves to the configured path instead of the default.
