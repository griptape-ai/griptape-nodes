...
convergence here -- and snapshotting node.metadata on entry so
post-aprocess checks can diff against it -- is how the strict-mode
framework enforces orchestrator/worker parity.
"""
...
class NodeExecutionSnapshot:
    """Snapshot of mutable-on-aprocess node state, taken pre-aprocess.

    Used by post-aprocess strict-mode checks (e.g.
    ``parameter-mutation-during-aprocess``) to diff what changed during
    execution. ``metadata`` is deep-copied so in-place mutations by the
    node do not alias the snapshot.
    """

    metadata: dict[str, Any]


async def prepare_node_for_aprocess(node: BaseNode, request: ExecuteNodeRequest) -> NodeExecutionSnapshot:
    ...
    4. Return a ``NodeExecutionSnapshot`` capturing ``node.metadata``
       for post-aprocess diffing.
    ...
    return NodeExecutionSnapshot(metadata=copy.deepcopy(node.metadata))
