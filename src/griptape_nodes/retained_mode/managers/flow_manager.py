...
class FlowManager:
    ...
    def on_create_connection_request(self, request: CreateConnectionRequest) -> ResultPayload:  # noqa: PLR0911, PLR0912, PLR0915, C901
        ...
        # Check if this connection is part of a loop structure and handle subflow management
        self._handle_loop_subflow_management(source_node, target_node)

        result = CreateConnectionResultSuccess()

        return result

    def _handle_loop_subflow_management(
        self, source_node: BaseNode, target_node: BaseNode
    ) -> None:
        ...
        """
        from griptape_nodes.exe_types.node_types import EndLoopNode, StartLoopNode
        ...
        elif isinstance(target_node, EndLoopNode) and target_node.subflow_name and not isinstance(source_node, StartLoopNode):
    ...
    def _move_node_between_flows(self, node:BaseNode, source_flow_name:str, target_flow_name:str) -> None:
    ...
    def _move_node_and_descendants_to_subflow(self, node: BaseNode, subflow_name: str, visited: set | None = None) -> None:
        ...
        for connection in flow.connections.get_outgoing_connections(node):
            ...
            if source_param.type == ParameterTypeBuiltin.CONTROL_TYPE.value:
    ...
    def _move_node_and_ancestors_to_subflow(self, node:BaseNode, subflow_name:str, visited:set|None=None) -> None:
