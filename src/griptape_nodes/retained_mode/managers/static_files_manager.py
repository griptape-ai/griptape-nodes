...
class StaticFilesManager:
    ...
    def on_handle_create_static_file_download_url_from_path_request(
        ...
        if request.preview:
            try:
                file_path_to_use = self._generate_preview_if_needed(full_file_path)
            except Exception as e:
                logger.warning("Preview generation failed for %s, using original: %s", request.file_path, e)
                file_path_to_use = full_file_path
        else:
            file_path_to_use = full_file_path
        ...
        try:
            ...
            url = self.storage_driver.create_signed_download_url(file_path_to_use)
        except Exception as e:
        ...
        return CreateStaticFileDownloadUrlResultSuccess(
            url=url,
            file_url=self.storage_driver.get_asset_url(file_path_to_use),
            result_details="Successfully created static file download URL",
        )
