...
class NodeManager:
    ...
    def _validate_and_handle_connections(
        self, node_name: str, parameter: Parameter, request: AlterParameterDetailsRequest
    ) -> None:
        """Validate and handle connections when parameter type changes."""
        try:
            ...
            for incoming_connection in list_connections_result.incoming_connections:
                if incoming_connection.target_parameter_name == request.parameter_name:
                    ...
                    source_param = source_node.get_parameter_by_name(incoming_connection.source_parameter_name)

                    # Check if the connection is still valid
                    if source_param is not None and not parameter.is_incoming_type_allowed(source_param.output_type):
            ...
            for outgoing_connection in list_connections_result.outgoing_connections:
                if outgoing_connection.source_parameter_name == request.parameter_name:
                    ...
                    target_param = target_node.get_parameter_by_name(outgoing_connection.target_parameter_name)

                    # Check if the connection is still valid
                    if target_param is not None and not target_param.is_incoming_type_allowed(parameter.output_type):
                        # Break the connection
                        logger.info(
                            "Breaking connection from %s.%s to %s.%s due to type mismatch. "
                            "Source type: %s, Target allowed types: %s",
    ...
    def on_alter_parameter_details_request(self, request: AlterParameterDetailsRequest) -> ResultPayload:
        ...
        if request.type is not None or request.input_types is not None or request.output_type is not None:
            self._validate_and_handle_connections(node_name, parameter, request)
