...
class LibraryVersionMinorDifferenceProblem(WorkflowProblem):
    ...
    def collate_problems_for_display(cls, instances: list[LibraryVersionMinorDifferenceProblem]) -> str:
        ...
        if len(instances) == 1:
            problem = instances[0]
            return f"This workflow was built with library '{problem.library_name}' v{problem.workflow_version}, but you have v{problem.current_version}. Minor differences are usually compatible. If you experience issues, you can update the library or save this workflow to update it to your current library versions."
        ...
        output_lines = []
        output_lines.append(
            f"Encountered {len(instances)} libraries with minor version differences (usually compatible):"
        )
