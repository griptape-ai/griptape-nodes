...
class LibraryProvisioningActionKind(StrEnum):
    """What provisioning will do to a single pinned library."""

    SKIP = "SKIP"  # installed version already satisfies the pin
...
class LibraryProvisioningSource(StrEnum):
    """Where a pinned library is installed from."""
...
class LibraryProvisioningAction:
    """The planned provisioning outcome for one pinned library.
    ...
    Fields:
        library_name: Library name, matching the pin's `name`.
        kind: SKIP / INSTALL / OVERWRITE.
        source: GIT or PYPI (inferred from which pin field is set).
        installed_version: Currently registered version, or None when not installed.
        pinned_version: The pin's PEP 440 version specifier, or None for a source-only pin.
        git_url: The pin's git source (url@ref form), when source is GIT.
...
class PreviewProjectProvisioningRequest(RequestPayload):
    ...
    Use when: the UI wants to show the user which pinned libraries will be
    installed or overwritten before committing to a project switch. Read-only:
    a registry read plus a PEP 440 compare, no clone/venv/delete work.

    Args:
        project_id: Identifier of an already-loaded project template to preview.
...
class PreviewProjectProvisioningResultSuccess(WorkflowNotAlteredMixin, ResultPayloadSuccess):
    ...
    Args:
        actions: One action per pinned library, in template order. Empty when
            the project declares no library pins.
    """
