...
# Metadata fields included in serialized event dicts for type routing/lookup
# that are not constructor parameters on event classes. These must be stripped
# before passing **event_data to an event constructor.
_EVENT_METADATA_KEYS = ("event_type", "request_type", "result_type", "payload_type")
...
class Payload(ABC):  # noqa: B024
    ...
    def to_json(self, **kwargs) -> str:
        ...
        """
        # Lazy import to avoid circular dependency between base_events and event_converter
        from griptape_nodes.retained_mode.events.event_converter import converter

        return json.dumps(converter.unstructure(self), default=str, **kwargs)
...
class EventRequest[P: Payload](BaseEvent):
    ...
    def dict(self, *args, **kwargs) -> dict[str, Any]:
        ...
        result = super().dict(*args, **kwargs)
        result["request"] = converter.unstructure(self.request)
        return result
...
class EventResult[P: RequestPayload, R: ResultPayload](BaseEvent, ABC):
    ...
    def dict(self, *args, **kwargs) -> dict[str, Any]:
        ...
        result = super().dict(*args, **kwargs)
        result["request"] = converter.unstructure(self.request)
        result["result"] = converter.unstructure(self.result)
...
class ExecutionEvent[E: ExecutionPayload](BaseEvent):
    ...
    def dict(self, *args, **kwargs) -> dict[str, Any]:
        ...
        result = super().dict(*args, **kwargs)
        result["payload"] = converter.unstructure(self.payload)
        return result
