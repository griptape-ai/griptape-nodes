...
if __name__ == "__main__":
    ...
    validator1 = TypeValidator.get_instance()
    validator2 = TypeValidator.get_instance()
    print(f"Are validators the same instance? {validator1 is validator2}")

    # Configure the validator
    TypeValidator.configure(allow_dynamic_imports=True)

    # Define a custom class for testing
    class Person:
        pass

    # Test type specification validation
    type_specs = [
        "int",  # Valid primitive type
        "bol",  # Invalid - not a known type
        "bool",  # Valid primitive type
        "str",  # Valid primitive type
        "List[int]",  # Valid container with valid type
        "Dict[str, Person]",  # Valid if Person is in scope
        "Set[invalid]",  # Invalid - 'invalid' is not a type
        "tuple[,]",  # Invalid syntax
        "MyCustomType",  # Invalid unless defined in scope
        "datetime.datetime",  # Valid standard library type
        "Optional[str]",  # Valid Union type
        "Union[int, str]",  # Valid Union type
        "Dict[str, List[int]]",  # Valid nested container
        "int | str",  # Python 3.10+ union syntax
        "dict[str, int | float]",  # Python 3.10+ nested union
    ]

    # Test single type validation (should not raise exceptions)
    print("Single type validation:")
    for type_str in type_specs[:3]:  # Test a few types
        is_valid = TypeValidator.validate_type_spec(type_str)
        print(f"  {type_str}: {'Valid' if is_valid else 'Invalid'}")

    # Test multiple type validation (should not raise exceptions)
    validation_results = TypeValidator.validate_type_specs(type_specs)
    print("\nMultiple type validation results:")
    for type_str, is_valid in validation_results.items():
        print(f"  {type_str}: {'Valid' if is_valid else 'Invalid'}")

    # Test single type conversion (will raise exceptions for invalid types)
    print("\nSingle type conversion (with exception handling):")
    for type_str in type_specs[:5]:  # Test a few types
        try:
            type_obj = TypeValidator.convert_to_type(type_str)
            print(f"  {type_str}: {type_obj}")
        except Exception as e:
            print(f"  {type_str}: Error - {e!s}")

    # Convert valid type specifications to Python types (with exception handling)
    print("\nConverting valid type specifications to Python types (with exception handling):")
    valid_type_specs = [type_str for type_str, valid in validation_results.items() if valid]
    try:
        type_objects = TypeValidator.convert_to_types(valid_type_specs)
        for type_str, type_obj in type_objects.items():
            print(f"  {type_str}: {type_obj}")
    except Exception as e:
        print(f"  Error: {e!s}")

    # Test is_instance functionality (with exception handling)
    print("\nTesting is_instance functionality (with exception handling):")
    test_values = [
        (42, "int"),  # True
        ("hello", "str"),  # True
        (True, "bool"),  # True
        ...
        (42, "int | str"),  # True (Python 3.10+ union)
        ("hello", "int | str"),  # True (Python 3.10+ union)
        ([1, 2], "int | str"),  # False (Python 3.10+ union)
    ]

    for value, type_str in test_values:
        try:
            result = TypeValidator.is_instance(value, type_str)
            print(f"  is_instance({value}, {type_str}) = {result}")
        except Exception as e:
            print(f"  is_instance({value}, {type_str}) = Error: {e!s}")

    # Demonstrate exception handling for invalid types
    print("\nDemonstrating exception handling for invalid types:")
    invalid_types = ["bol", "Set[invalid]", "tuple[,]", "MyCustomType"]

    for type_str in invalid_types:
        try:
            print(f"  Converting '{type_str}'...")
            TypeValidator.convert_to_type(type_str)
            print("  Success (unexpected)")
        except Exception as e:
            print(f"  Expected error: {type(e).__name__}: {e!s}")
